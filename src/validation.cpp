...
const string strMessageMagic = "usdecoin Signed Message:\n";
...
int GetInputAgeIX(const uint256 &nTXHash, const CTxIn &txin)
{
    ...
    if (nResult < 6 && instapay.IsLockedInstaPayTransaction(nTXHash))
...
{
    if (instapay.IsLockedInstaPayTransaction(nTXHash))
        return nInstaPayDepth;
                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
    ...
    if(instapay.HasTxLockRequest(hash) && !CTxLockRequest(tx).IsValid())
    ...
    {
        uint256 hashLocked;
        if(instapay.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hash != hashLocked)
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        if (pool.mapNextTx.count(txin.prevout))
        {
            ...
            {
                // InstaPay txes are not replacable
                if(instapay.HasTxLockRequest(ptxConflicting->GetHash())) {
                ...
                } else if (instapay.HasTxLockRequest(hash)) {
...
{
    CAmount ret = blockValue/5*3; // start at 40%
...
void ThreadScriptCheck() {
    RenameThread("usdecoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // USDE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(USDE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(USDE): couldn't find masternode or superblock payments"),
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    // Check timestamp
    if (block.GetBlockTime() > GetAdjustedTime() + 5 * 60)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // USDE : CHECK TRANSACTIONS FOR INSTAPAY

    if(sporkManager.IsSporkActive(SPORK_3_INSTAPAY_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                uint256 hashLocked;
                if(instapay.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    instapay.Relay(hashLocked);
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(USDE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(USDE): spork is off, skipping transaction locking checks\n");
    }
