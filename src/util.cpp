...
const char * const BITCOIN_CONF_FILENAME = "usdecoin.conf";
const char * const BITCOIN_PID_FILENAME = "usdecoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "usdecoin" is a composite category enabling all usdecoin-related debug output
            if(ptrCategory->count(string("usdecoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\usdecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\usdecoin
    // Mac: ~/Library/Application Support/usdecoin
    // Unix: ~/.usdecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "usdecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/usdecoin";
#else
    // Unix
    return pathRet / ".usdecoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty usdecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override usdecoin.conf
