...
UniValue privatepay(const UniValue& params, bool fHelp)
{
    ...
    if(params[0].get_str() == "start") {
        ...
        privatePayClient.fEnablePrivatePay = true;
        bool result = privatePayClient.DoAutomaticDenominating(*g_connman);
        return "Mixing " + (result ? "started successfully" : ("start failed: " + privatePayClient.GetStatus() + ", will retry"));
    }
    ...
    return "Unknown command, please see \"help privatepay\"";
}
...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    ...
    CPrivatePayBase privatePay = fMasterNode ? (CPrivatePayBase)privatePayServer : (CPrivatePayBase)privatePayClient;

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("state",             privatePay.GetStateString()));
    obj.push_back(Pair("mixing_mode",       (!fMasterNode && privatePayClient.fPrivatePayMultiSession) ? "multi-session" : "normal"));
    obj.push_back(Pair("queue",             privatePay.GetQueueSize()));
    obj.push_back(Pair("entries",           privatePay.GetEntriesCount()));
    obj.push_back(Pair("status",            privatePayClient.GetStatus()));

    if (privatePayClient.infoMixingMasternode.fInfoValid) {
        obj.push_back(Pair("outpoint",      privatePayClient.infoMixingMasternode.vin.prevout.ToStringShort()));
        obj.push_back(Pair("addr",          privatePayClient.infoMixingMasternode.addr.ToString()));
    }

    if (pwalletMain) {
        obj.push_back(Pair("keys_left",     pwalletMain->nKeysLeftSinceAutoBackup));
        obj.push_back(Pair("warnings",      pwalletMain->nKeysLeftSinceAutoBackup < PRIVATEPAY_KEYS_THRESHOLD_WARNING
...
UniValue masternode(const UniValue& params, bool fHelp)
{
        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot masternode configured in usdecoin.conf\n"
    ...
    if (strCommand == "count")
    {
        ...
        if (strMode == "ps")
            return mnodeman.CountEnabled(MIN_PRIVATEPAY_PEER_PROTO_VERSION);
        ...
        if (strMode == "all")
            return strprintf("Total: %d (PS Compatible: %d / Enabled: %d / Qualify: %d)",
                mnodeman.size(), mnodeman.CountEnabled(MIN_PRIVATEPAY_PEER_PROTO_VERSION),
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print usdecoin address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
