...
void CMasternodeMan::AskForMN(CNode* pnode, const CTxIn &vin)
{
    ...
    mWeAskedForMasternodeListEntry[vin.prevout][pnode->addr] = GetTime() + PPEG_UPDATE_SECONDS;

    g_connman->PushMessage(pnode, NetMsgType::PPEG, vin);
}
...
void CMasternodeMan::DsegUpdate(CNode* pnode)
{
    ...
    g_connman->PushMessage(pnode, NetMsgType::PPEG, CTxIn());
    int64_t askAgain = GetTime() + PPEG_UPDATE_SECONDS;
...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    g_connman->ForEachNode(CConnman::AllNodes, [](CNode* pnode) {
        if(pnode->fMasternode) {
            if(privatePayClient.infoMixingMasternode.fInfoValid && pnode->addr == privatePayClient.infoMixingMasternode.addr) return true;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
    ...
    } else if (strCommand == NetMsgType::PPEG) { //Get Masternode list or specific entry
        ...
        LogPrint("masternode", "PPEG -- Masternode list, masternode=%s\n", vin.prevout.ToStringShort());
        ...
        if(vin == CTxIn()) { //only should ask for this once
            ...
            if(!isLocal && Params().NetworkIDString() == CBaseChainParams::MAIN) {
                ...
                if (it != mAskedUsForMasternodeList.end() && it->second > GetTime()) {
                    ...
                    LogPrintf("PPEG -- peer already asked me for the list, peer=%d\n", pfrom->id);
                    return;
                }
                int64_t askAgain = GetTime() + PPEG_UPDATE_SECONDS;
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            LogPrint("masternode", "PPEG -- Sending Masternode entry: masternode=%s  addr=%s\n", mn.vin.prevout.ToStringShort(), mn.addr.ToString());
            ...
            if (vin == mn.vin) {
                LogPrintf("PPEG -- Sent 1 Masternode inv to peer %d\n", pfrom->id);
                return;
            }
        }
        ...
        if(vin == CTxIn()) {
            ...
            LogPrintf("PPEG -- Sent %d Masternode invs to peer %d\n", nInvCount, pfrom->id);
            return;
        }
        // smth weird happen - someone asked us for vin we have no idea about?
        LogPrint("masternode", "PPEG -- No invs sent to peer %d\n", pfrom->id);
