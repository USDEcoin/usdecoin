...
// Copyright (c) 2017-2017 The usdecoin Core developers
...
bool fEnableInstaPay = true;
int nInstaPayDepth = DEFAULT_INSTAPAY_DEPTH;
...
void CInstaPay::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if(fLiteMode) return; // disable all usdecoin specific functionality
    if(!sporkManager.IsSporkActive(SPORK_2_INSTAPAY_ENABLED)) return;

    // Ignore any InstaPay messages until masternode list is synced
    if(!masternodeSync.IsMasternodeListSynced()) return;

    // NOTE: NetMsgType::TXLOCKREQUEST is handled via ProcessMessage() in main.cpp

    if (strCommand == NetMsgType::TXLOCKVOTE) // InstaPay Transaction Lock Consensus Votes
    {
        if(pfrom->nVersion < MIN_INSTAPAY_PROTO_VERSION) return;
...
bool CInstaPay::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK2(cs_main, cs_instapay);
    ...
    BOOST_FOREACH(const CTxIn& txin, txLockRequest.vin) {
        ...
        if(it != mapLockedOutpoints.end()) {
            ...
            LogPrintf("CInstaPay::ProcessTxLockRequest -- WARNING: Found conflicting completed Transaction Lock, skipping current one, txid=%s, completed lock txid=%s\n",
        ...
        if(it != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, it->second) {
                if(hash != txLockRequest.GetHash()) {
                    LogPrint("instapay", "CInstaPay::ProcessTxLockRequest -- Double spend attempt! %s\n", txin.prevout.ToStringShort());
    ...
    if(!CreateTxLockCandidate(txLockRequest)) {
        // smth is not right
        LogPrintf("CInstaPay::ProcessTxLockRequest -- CreateTxLockCandidate failed, txid=%s\n", txHash.ToString());
        return false;
    }
    LogPrintf("CInstaPay::ProcessTxLockRequest -- accepted, txid=%s\n", txHash.ToString());
...
bool CInstaPay::CreateTxLockCandidate(const CTxLockRequest& txLockRequest)
{
    ...
    if(itLockCandidate == mapTxLockCandidates.end()) {
        LogPrintf("CInstaPay::CreateTxLockCandidate -- new, txid=%s\n", txHash.ToString());
        ...
        LogPrint("instapay", "CInstaPay::CreateTxLockCandidate -- seen, txid=%s\n", txHash.ToString());
    }

    return true;
}

void CInstaPay::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        if(nPrevoutHeight == -1) {
            LogPrint("instapay", "CInstaPay::Vote -- Failed to find UTXO %s\n", itOutpointLock->first.ToStringShort());
            return;
        }

        int nLockInputHeight = nPrevoutHeight + 4;

        int n = mnodeman.GetMasternodeRank(activeMasternode.vin, nLockInputHeight, MIN_INSTAPAY_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instapay", "CInstaPay::Vote -- Can't calculate rank for masternode %s\n", activeMasternode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instapay", "CInstaPay::Vote -- Masternode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }

        LogPrint("instapay", "CInstaPay::Vote -- In the top %d (%d)\n", nSignaturesTotal, n);
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasMasternodeVoted(itOutpointLock->first, activeMasternode.vin.prevout)) {
                    ...
                    // skip it anyway
                    fAlreadyVoted = true;
                    LogPrintf("CInstaPay::Vote -- WARNING: We already voted for this outpoint, skipping: txHash=%s, outpoint=%s\n",
        ...
        if(!vote.Sign()) {
            LogPrintf("CInstaPay::Vote -- Failed to sign consensus vote\n");
            return;
        }
        if(!vote.CheckSignature()) {
            LogPrintf("CInstaPay::Vote -- Signature invalid\n");
            return;
        }
        ...
        if(itOutpointLock->second.AddVote(vote)) {
            LogPrintf("CInstaPay::Vote -- Vote created successfully, relaying: txHash=%s, outpoint=%s, vote=%s\n",
            ...
            if(itVoted == mapVotedOutpoints.end()) {
                ...
                if(mapVotedOutpoints[itOutpointLock->first].size() > 1) {
                    ...
                    LogPrintf("CInstaPay::Vote -- WARNING: Vote conflicts with some existing votes: txHash=%s, outpoint=%s, vote=%s\n",
                            txHash.ToString(), itOutpointLock->first.ToStringShort(), nVoteHash.ToString());
                }
            }
...
bool CInstaPay::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    // cs_main, cs_wallet and cs_instapay should be already locked
    AssertLockHeld(cs_main);
#ifdef ENABLE_WALLET
    if (pwalletMain)
        AssertLockHeld(pwalletMain->cs_wallet);
#endif
    AssertLockHeld(cs_instapay);

    uint256 txHash = vote.GetTxHash();

    if(!vote.IsValid(pfrom)) {
        // could be because of missing MN
        LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- Vote is invalid, txid=%s\n", txHash.ToString());
        return false;
    }
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- Orphan vote: txid=%s  masternode=%s new\n",
            ...
            if(fReprocess && IsEnoughOrphanVotesForTx(itLockRequest->second)) {
                ...
                LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- Found enough orphan votes, reprocessing Transaction Lock Request: txid=%s\n", txHash.ToString());
                ProcessTxLockRequest(itLockRequest->second);
                return true;
            }
        } else {
            LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- Orphan vote: txid=%s  masternode=%s seen\n",
                    txHash.ToString(), vote.GetMasternodeOutpoint().ToStringShort());
        }
        ...
        if(!mapMasternodeOrphanVotes.count(vote.GetMasternodeOutpoint())) {
            ...
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageMasternodeOrphanVoteTime()) {
                LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- masternode is spamming orphan Transaction Lock Votes: txid=%s  masternode=%s\n",
                        txHash.ToString(), vote.GetMasternodeOutpoint().ToStringShort());
    ...
    LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- Transaction Lock Vote, txid=%s\n", txHash.ToString());
        ...
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                if(it2->second.HasMasternodeVoted(vote.GetOutpoint(), vote.GetMasternodeOutpoint())) {
                    ...
                    // pose ban score too.
                    LogPrintf("CInstaPay::ProcessTxLockVote -- masternode sent conflicting votes! %s\n", vote.GetMasternodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
    ...
    LogPrint("instapay", "CInstaPay::ProcessTxLockVote -- Transaction Lock signatures count: %d/%d, vote hash=%s\n",
...
void CInstaPay::ProcessOrphanTxLockVotes()
{
    LOCK(cs_main);
#ifdef ENABLE_WALLET
    if (pwalletMain)
...
bool CInstaPay::IsEnoughOrphanVotesForTx(const CTxLockRequest& txLockRequest)
{
...
bool CInstaPay::IsEnoughOrphanVotesForTxAndOutPoint(const uint256& txHash, const COutPoint& outpoint)
{
...
void CInstaPay::TryToFinalizeLockCandidate(const CTxLockCandidate& txLockCandidate)
{
    LOCK(cs_main);
#ifdef ENABLE_WALLET
    if (pwalletMain)
    ...
    if(txLockCandidate.IsAllOutPointsReady() && !IsLockedInstaPayTransaction(txHash)) {
        // we have enough votes now
        LogPrint("instapay", "CInstaPay::TryToFinalizeLockCandidate -- Transaction Lock is ready to complete, txid=%s\n", txHash.ToString());
        if(ResolveConflicts(txLockCandidate, Params().GetConsensus().nInstaPayKeepLock)) {
...
void CInstaPay::UpdateLockedTransaction(const CTxLockCandidate& txLockCandidate)
{
    // cs_wallet and cs_instapay should be already locked
#ifdef ENABLE_WALLET
    if (pwalletMain)
        AssertLockHeld(pwalletMain->cs_wallet);
#endif
    ...
    if(!IsLockedInstaPayTransaction(txHash)) return; // not a locked tx, do not update/notify
    ...
    if(pwalletMain && pwalletMain->UpdatedTransaction(txHash)) {
        ...
        std::string strCmd = GetArg("-instapaynotify", "");
        if(!strCmd.empty()) {
    ...
    LogPrint("instapay", "CInstaPay::UpdateLockedTransaction -- done, txid=%s\n", txHash.ToString());
}

void CInstaPay::LockTransactionInputs(const CTxLockCandidate& txLockCandidate)
{
    LOCK(cs_instapay);
    ...
    LogPrint("instapay", "CInstaPay::LockTransactionInputs -- done, txid=%s\n", txHash.ToString());
}

bool CInstaPay::GetLockedOutPointTxHash(const COutPoint& outpoint, uint256& hashRet)
{
    LOCK(cs_instapay);
...
bool CInstaPay::ResolveConflicts(const CTxLockCandidate& txLockCandidate, int nMaxBlocks)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, txLockCandidate.txLockRequest.vin) {
        ...
        if(GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            LogPrintf("CInstaPay::ResolveConflicts -- WARNING: Found conflicting completed Transaction Lock, skipping current one, txid=%s, conflicting txid=%s\n",
            ...
            if(HasTxLockRequest(hashConflicting)) {
                ...
                LogPrintf("CInstaPay::ResolveConflicts -- WARNING: Found conflicting Transaction Lock Request, replacing by completed Transaction Lock, txid=%s, conflicting txid=%s\n",
                        txHash.ToString(), hashConflicting.ToString());
            } else {
                // If this lock is completed, we don't really care about normal conflicting txes.
                LogPrintf("CInstaPay::ResolveConflicts -- WARNING: Found conflicting transaction, replacing by completed Transaction Lock, txid=%s, conflicting txid=%s\n",
                        txHash.ToString(), hashConflicting.ToString());
            }
        }
    } // FOREACH
    if(fMempoolConflict) {
        ...
        if(!AcceptToMemoryPool(mempool, state, txLockCandidate.txLockRequest, true, &fMissingInputs)) {
            LogPrintf("CInstaPay::ResolveConflicts -- ERROR: Failed to accept completed Transaction Lock to mempool, txid=%s\n", txHash.ToString());
            return false;
        }
        LogPrintf("CInstaPay::ResolveConflicts -- Accepted completed Transaction Lock, txid=%s\n", txHash.ToString());
        return true;
    }
    ...
    if(GetTransaction(txHash, txTmp, Params().GetConsensus(), hashBlock, true) && hashBlock != uint256()) {
        LogPrint("instapay", "CInstaPay::ResolveConflicts -- Done, %s is included in block %s\n", txHash.ToString(), hashBlock.ToString());
        return true;
    }
    ...
    LogPrint("instapay", "CInstaPay::ResolveConflicts -- Done, txid=%s\n", txHash.ToString());

    return true;
}

int64_t CInstaPay::GetAverageMasternodeOrphanVoteTime()
{
    LOCK(cs_instapay);
...
void CInstaPay::CheckAndRemove()
{
    ...
    while(itLockCandidate != mapTxLockCandidates.end()) {
        ...
        if(txLockCandidate.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrintf("CInstaPay::CheckAndRemove -- Removing expired Transaction Lock Candidate: txid=%s\n", txHash.ToString());
            std::map<COutPoint, COutPointLock>::iterator itOutpointLock = txLockCandidate.mapOutPointLocks.begin();
            while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instapay", "CInstaPay::CheckAndRemove -- Removing expired vote: txid=%s  masternode=%s\n",
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instapay", "CInstaPay::CheckAndRemove -- Removing expired orphan vote: txid=%s  masternode=%s\n",
    ...
    while(itMasternodeOrphan != mapMasternodeOrphanVotes.end()) {
        if(itMasternodeOrphan->second < GetTime()) {
            LogPrint("instapay", "CInstaPay::CheckAndRemove -- Removing expired orphan masternode vote: masternode=%s\n",
    ...
    LogPrintf("CInstaPay::CheckAndRemove -- %s\n", ToString());
}

bool CInstaPay::AlreadyHave(const uint256& hash)
{
    LOCK(cs_instapay);
...
void CInstaPay::AcceptLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK(cs_instapay);
    mapLockRequestAccepted.insert(make_pair(txLockRequest.GetHash(), txLockRequest));
}

void CInstaPay::RejectLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK(cs_instapay);
    mapLockRequestRejected.insert(make_pair(txLockRequest.GetHash(), txLockRequest));
}

bool CInstaPay::HasTxLockRequest(const uint256& txHash)
{
    CTxLockRequest txLockRequestTmp;
    return GetTxLockRequest(txHash, txLockRequestTmp);
}

bool CInstaPay::GetTxLockRequest(const uint256& txHash, CTxLockRequest& txLockRequestRet)
{
    LOCK(cs_instapay);
...
bool CInstaPay::GetTxLockVote(const uint256& hash, CTxLockVote& txLockVoteRet)
{
    LOCK(cs_instapay);
...
bool CInstaPay::IsInstaPayReadyToLock(const uint256& txHash)
{
    if(!fEnableInstaPay || fLargeWorkForkFound || fLargeWorkInvalidChainFound ||
        !sporkManager.IsSporkActive(SPORK_2_INSTAPAY_ENABLED)) return false;
...
bool CInstaPay::IsLockedInstaPayTransaction(const uint256& txHash)
{
    if(!fEnableInstaPay || fLargeWorkForkFound || fLargeWorkInvalidChainFound ||
        !sporkManager.IsSporkActive(SPORK_2_INSTAPAY_ENABLED)) return false;
...
int CInstaPay::GetTransactionLockSignatures(const uint256& txHash)
{
    if(!fEnableInstaPay) return -1;
    if(fLargeWorkForkFound || fLargeWorkInvalidChainFound) return -2;
    if(!sporkManager.IsSporkActive(SPORK_2_INSTAPAY_ENABLED)) return -3;
...
bool CInstaPay::IsTxLockRequestTimedOut(const uint256& txHash)
{
    if(!fEnableInstaPay) return false;
...
void CInstaPay::Relay(const uint256& txHash)
{
    LOCK(cs_instapay);
...
void CInstaPay::UpdatedBlockTip(const CBlockIndex *pindex)
{
    pCurrentBlockIndex = pindex;
}

void CInstaPay::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    if(pblock) {
        ...
        if(mi == mapBlockIndex.end() || !mi->second) {
            // shouldn't happen
            LogPrint("instapay", "CTxLockRequest::SyncTransaction -- Failed to find block %s\n", blockHash.ToString());
            return;
        }
    ...
    LogPrint("instapay", "CInstaPay::SyncTransaction -- txid=%s nHeightNew=%d\n", txHash.ToString(), nHeightNew);

    // Check lock candidates
    std::map<uint256, CTxLockCandidate>::iterator itLockCandidate = mapTxLockCandidates.find(txHash);
    if(itLockCandidate != mapTxLockCandidates.end()) {
        LogPrint("instapay", "CInstaPay::SyncTransaction -- txid=%s nHeightNew=%d lock candidate updated\n",
                txHash.ToString(), nHeightNew);
        ...
        while(itOutpointLock != itLockCandidate->second.mapOutPointLocks.end()) {
            ...
            while(itVote != vVotes.end()) {
                ...
                LogPrint("instapay", "CInstaPay::SyncTransaction -- txid=%s nHeightNew=%d vote %s updated\n",
...
bool CTxLockRequest::IsValid(bool fRequireUnspent) const
{
    ...
    if(vin.size() > WARN_MANY_INPUTS) {
        LogPrint("instapay", "CTxLockRequest::IsValid -- WARNING: Too many inputs: tx=%s", ToString());
    }

    LOCK(cs_main);
    if(!CheckFinalTx(*this)) {
        LogPrint("instapay", "CTxLockRequest::IsValid -- Transaction is not final: tx=%s", ToString());
        return false;
    }
    ...
    BOOST_FOREACH(const CTxOut& txout, vout) {
        // InstaPay supports normal scripts and unspendable (i.e. data) scripts.
        // TODO: Look into other script types that are normal and can be included
        if(!txout.scriptPubKey.IsNormalPaymentScript() && !txout.scriptPubKey.IsUnspendable()) {
            LogPrint("instapay", "CTxLockRequest::IsValid -- Invalid Script %s", ToString());
            return false;
        }
    ...
    BOOST_FOREACH(const CTxIn& txin, vin) {
        ...
        if(!pcoinsTip->GetCoins(txin.prevout.hash, coins) ||
           (unsigned int)txin.prevout.n>=coins.vout.size() ||
           coins.vout[txin.prevout.n].IsNull()) {
            LogPrint("instapay", "CTxLockRequest::IsValid -- Failed to find UTXO %s\n", txin.prevout.ToStringShort());
            ...
            if(!GetTransaction(txin.prevout.hash, txOutpointCreated, Params().GetConsensus(), nHashOutpointConfirmed, true) || nHashOutpointConfirmed == uint256()) {
                LogPrint("instapay", "CTxLockRequest::IsValid -- Failed to find outpoint %s\n", txin.prevout.ToStringShort());
                return false;
            }
            if(txin.prevout.n >= txOutpointCreated.vout.size()) {
                LogPrint("instapay", "CTxLockRequest::IsValid -- Outpoint %s is out of bounds, size() = %lld\n",
            ...
            if(mi == mapBlockIndex.end() || !mi->second) {
                // shouldn't happen
                LogPrint("instapay", "CTxLockRequest::IsValid -- Failed to find block %s for outpoint %s\n",
        ...
        int nConfirmationsRequired = INSTAPAY_CONFIRMATIONS_REQUIRED - 1;

        if(nTxAge < nConfirmationsRequired) {
            LogPrint("instapay", "CTxLockRequest::IsValid -- outpoint %s too new: nTxAge=%d, nConfirmationsRequired=%d, txid=%s\n",
    ...
    if(nValueOut > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
        LogPrint("instapay", "CTxLockRequest::IsValid -- Transaction value too high: nValueOut=%d, tx=%s", nValueOut, ToString());
        return false;
    }

    if(nValueIn - nValueOut < GetMinFee()) {
        LogPrint("instapay", "CTxLockRequest::IsValid -- did not include enough fees in transaction: fees=%d, tx=%s", nValueOut - nValueIn, ToString());
        return false;
    }
...
bool CTxLockVote::IsValid(CNode* pnode) const
{
    if(!mnodeman.Has(CTxIn(outpointMasternode))) {
        LogPrint("instapay", "CTxLockVote::IsValid -- Unknown masternode %s\n", outpointMasternode.ToStringShort());
    ...
    if(nPrevoutHeight == -1) {
        LogPrint("instapay", "CTxLockVote::IsValid -- Failed to find UTXO %s\n", outpoint.ToStringShort());
        ...
        if(!GetTransaction(outpoint.hash, txOutpointCreated, Params().GetConsensus(), nHashOutpointConfirmed, true) || nHashOutpointConfirmed == uint256()) {
            LogPrint("instapay", "CTxLockVote::IsValid -- Failed to find outpoint %s\n", outpoint.ToStringShort());
            return false;
        }
        LOCK(cs_main);
        BlockMap::iterator mi = mapBlockIndex.find(nHashOutpointConfirmed);
        if(mi == mapBlockIndex.end() || !mi->second) {
            // not on this chain?
            LogPrint("instapay", "CTxLockVote::IsValid -- Failed to find block %s for outpoint %s\n", nHashOutpointConfirmed.ToString(), outpoint.ToStringShort());
            return false;
        }
        nPrevoutHeight = mi->second->nHeight;
    }
    ...
    int n = mnodeman.GetMasternodeRank(CTxIn(outpointMasternode), nLockInputHeight, MIN_INSTAPAY_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instapay", "CTxLockVote::IsValid -- Can't calculate rank for masternode %s\n", outpointMasternode.ToStringShort());
        return false;
    }
    LogPrint("instapay", "CTxLockVote::IsValid -- Masternode %s, rank=%d\n", outpointMasternode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instapay", "CTxLockVote::IsValid -- Masternode %s is not in the top %d (%d), vote hash=%s\n",
...
{
    // Locks and votes expire nInstaPayKeepLock blocks after the block corresponding tx was included into.
    return (nConfirmedHeight != -1) && (nHeight - nConfirmedHeight > Params().GetConsensus().nInstaPayKeepLock);
}
