...
extern const char *PPACCEPT;
extern const char *PPVIN;
extern const char *PPFINALTX;
extern const char *PPSIGNFINALTX;
extern const char *PPCOMPLETE;
extern const char *PPSTATUSUPDATE;
extern const char *PPTX;
extern const char *PPQUEUE;
extern const char *PPEG;
...
enum ServiceFlags : uint64_t {
    ...
    // set by all usdecoin Core nodes, and is unset by SPV clients or other peers that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // usdecoin Core does not support this but a patch set called Bitcoin XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // usdecoin Core nodes used to support this by default, without advertising this bit,
