...
// timeouts
static const int PRIVATEPAY_AUTO_TIMEOUT_MIN       = 5;
static const int PRIVATEPAY_AUTO_TIMEOUT_MAX       = 15;
static const int PRIVATEPAY_QUEUE_TIMEOUT          = 30;
static const int PRIVATEPAY_SIGNING_TIMEOUT        = 15;

//! minimum peer version accepted by mixing pool
static const int MIN_PRIVATEPAY_PEER_PROTO_VERSION = 70206;

static const CAmount PRIVATEPAY_ENTRY_MAX_SIZE     = 9;
...
public:
    ...
    CPrivatePayEntry(const std::vector<CTxIn>& vecTxIn, const std::vector<CTxOut>& vecTxOut, const CTransaction& txCollateral);
...
public:
    ...
    CPrivatepayQueue(int nDenom, CTxIn vin, int64_t nTime, bool fReady) :
        nDenom(nDenom),
        vin(vin),
        nTime(nTime),
        fReady(fReady),
    ...
    bool IsExpired() { return GetTime() - nTime > PRIVATEPAY_QUEUE_TIMEOUT; }
    ...
    friend bool operator==(const CPrivatepayQueue& a, const CPrivatepayQueue& b)
    {
...
public:
    ...
    CPrivatepayBroadcastTx(CTransaction tx, CTxIn vin, int64_t sigTime) :
        nConfirmedHeight(-1),
        tx(tx),
        vin(vin),
        vchSig(),
    ...
    friend bool operator==(const CPrivatepayBroadcastTx& a, const CPrivatepayBroadcastTx& b)
    {
        return a.tx == b.tx;
    }
    friend bool operator!=(const CPrivatepayBroadcastTx& a, const CPrivatepayBroadcastTx& b)
    ...
    {
        return *this != CPrivatepayBroadcastTx();
    }
...
// base class
class CPrivatePayBase
{
protected:
    // The current mixing sessions in progress on the network
    std::vector<CPrivatepayQueue> vecPrivatepayQueue;

    std::vector<CPrivatePayEntry> vecEntries; // Masternode/clients entries
...
public:
    ...
    int GetQueueSize() const { return vecPrivatepayQueue.size(); }
...
private:
    ...
    CPrivatePay() {}
    ~CPrivatePay() {}
    CPrivatePay(CPrivatePay const&) = delete;
    CPrivatePay& operator= (CPrivatePay const&) = delete;

    static const CAmount COLLATERAL = 0.001 * COIN;

    // static members
    static std::vector<CAmount> vecStandardDenominations;
    static std::map<uint256, CPrivatepayBroadcastTx> mapPPTX;

    static CCriticalSection cs_mappptx;
...
public:
    ...
    /// Get the denominations for a specific amount of usdecoin.
    ...
    static CAmount GetMaxPoolAmount() { return vecStandardDenominations.empty() ? 0 : PRIVATEPAY_ENTRY_MAX_SIZE * vecStandardDenominations.front(); }
    ...
    static void AddPPTX(const CPrivatepayBroadcastTx& pptx);
    static CPrivatepayBroadcastTx GetPPTX(const uint256& hash);
    static void CheckPPTXes(int nHeight);
