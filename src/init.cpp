...
#include "config/usdecoin-config.h"
#endif
...
static CPPNotificationInterface* pppNotificationInterface = NULL;
...
void PrepareShutdown()
{
    ...
    if (pppNotificationInterface) {
        UnregisterValidationInterface(pppNotificationInterface);
        delete pppNotificationInterface;
        pppNotificationInterface = NULL;
    }
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via InstaPay) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via InstaPay) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "usdecoin (or specifically: privatepay, instapay, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all usdecoin specific functionality (Masternodes, PrivatePay, InstaPay, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("PrivatePay options:"));
    ...
    strUsage += HelpMessageOpt("-privatepayamount=<n>", strprintf(_("Keep N USDE anonymized (default: %u)"), DEFAULT_PRIVATEPAY_AMOUNT));
    ...
    strUsage += HelpMessageOpt("-enableinstapay=<n>", strprintf(_("Enable InstaPay, show confirmations for locked transactions (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-instapaydepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), DEFAULT_INSTAPAY_DEPTH));
    strUsage += HelpMessageOpt("-instapaynotify=<cmd>", _("Execute command when a wallet InstaPay transaction is successfully locked (%s in cmd is replaced by TxID)"));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The usdecoin Core Developers SK"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that usdecoin Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if(!GetBoolArg("-enableinstapay", fEnableInstaPay)){
        if (SoftSetArg("-instapaydepth", 0))
            LogPrintf("%s: parameter interaction: -enableinstapay=false -> setting -nInstaPayDepth=0\n", __func__);
    }

    int nLiqProvTmp = GetArg("-liquidityprovider", DEFAULT_PRIVATEPAY_LIQUIDITY);
    if (nLiqProvTmp > 0) {
        mapArgs["-enableprivatepay"] = "1";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -enableprivatepay=1\n", __func__, nLiqProvTmp);
        mapArgs["-privatepayrounds"] = "99999";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatepayrounds=99999\n", __func__, nLiqProvTmp);
        mapArgs["-privatepayamount"] = "999999";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatepayamount=999999\n", __func__, nLiqProvTmp);
        mapArgs["-privatepaymultisession"] = "0";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatepaymultisession=0\n", __func__, nLiqProvTmp);
    }
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("usdecoin Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize usdecoin Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. usdecoin Core is shutting down."));
...
#endif
    // Make sure only a single usdecoin Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. usdecoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. usdecoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    pppNotificationInterface = new CPPNotificationInterface();
    RegisterValidationInterface(pppNotificationInterface);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of usdecoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart usdecoin Core to complete") << "\n";
    ...
    privatePayClient.nLiquidityProvider = std::min(std::max((int)GetArg("-liquidityprovider", DEFAULT_PRIVATEPAY_LIQUIDITY), 0), 100);
    privatePayClient.SetMinBlockSpacing(privatePayClient.nLiquidityProvider * 15);

    privatePayClient.fEnablePrivatePay = GetBoolArg("-enableprivatepay", false);
    privatePayClient.fPrivatePayMultiSession = GetBoolArg("-privatepaymultisession", DEFAULT_PRIVATEPAY_MULTISESSION);
    privatePayClient.nPrivatePayRounds = std::min(std::max((int)GetArg("-privatepayrounds", DEFAULT_PRIVATEPAY_ROUNDS), 2), privatePayClient.nLiquidityProvider ? 99999 : 16);
    privatePayClient.nPrivatePayAmount = std::min(std::max((int)GetArg("-privatepayamount", DEFAULT_PRIVATEPAY_AMOUNT), 2), 999999);

    fEnableInstaPay = GetBoolArg("-enableinstapay", 1);
    nInstaPayDepth = GetArg("-instapaydepth", DEFAULT_INSTAPAY_DEPTH);
    nInstaPayDepth = std::min(std::max(nInstaPayDepth, 0), 60);

    //lite mode disables all Masternode and Privatepay related functionality
    ...
    LogPrintf("nInstaPayDepth %d\n", nInstaPayDepth);
    LogPrintf("PrivatePay rounds %d\n", privatePayClient.nPrivatePayRounds);
    LogPrintf("PrivatePay amount %d\n", privatePayClient.nPrivatePayAmount);

    CPrivatePay::InitStandardDenominations();
    ...
    // ********************************************************* Step 11c: update block tip in usdecoin modules

    // force UpdatedBlockTip to initialize pCurrentBlockIndex for DS, MN payments and budgets
    // but don't call it directly to prevent triggering of other listeners like zmq etc.
    // GetMainSignals().UpdatedBlockTip(chainActive.Tip());
    mnodeman.UpdatedBlockTip(chainActive.Tip());
    privatePayClient.UpdatedBlockTip(chainActive.Tip());
    mnpayments.UpdatedBlockTip(chainActive.Tip());
    masternodeSync.UpdatedBlockTip(chainActive.Tip(), IsInitialBlockDownload());
    governance.UpdatedBlockTip(chainActive.Tip());

    // ********************************************************* Step 11d: start usdecoin-ps-<smth> threads

    threadGroup.create_thread(boost::bind(&ThreadCheckPrivatePay));
    if (fMasterNode)
        threadGroup.create_thread(boost::bind(&ThreadCheckPrivatePayServer));
    else
        threadGroup.create_thread(boost::bind(&ThreadCheckPrivatePayClient, boost::ref(*g_connman)));
