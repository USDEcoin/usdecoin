...
#if defined(NDEBUG)
# error "usdecoin Core cannot be compiled without assertions."
#endif
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        usdecoin Related Inventory Messages
    ...
    */
    case MSG_TXLOCK_REQUEST:
        return instapay.AlreadyHave(inv.hash);
    ...
    case MSG_PPTX: {
        return static_cast<bool>(CPrivatePay::GetPPTX(inv.hash));
    }
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_TXLOCK_REQUEST) {
                    ...
                    if(instapay.GetTxLockRequest(inv.hash, txLockRequest)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << txLockRequest;
                ...
                if (!pushed && inv.type == MSG_TXLOCK_VOTE) {
                    CTxLockVote vote;
                    if(instapay.GetTxLockVote(inv.hash, vote)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << vote;
                ...
                if (!pushed && inv.type == MSG_PPTX) {
                    CPrivatepayBroadcastTx pptx = CPrivatePay::GetPPTX(inv.hash);
                    if(pptx) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << pptx;
                        connman.PushMessage(pfrom, NetMsgType::PPTX, ss);
                        pushed = true;
                    }
                }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::PPTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        } else if (strCommand == NetMsgType::PPTX) {
            vRecv >> pptx;
            tx = pptx.tx;
            nInvType = MSG_PPTX;
        }
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            if(!instapay.ProcessTxLockRequest(txLockRequest)) {
                LogPrint("instapay", "TXLOCKREQUEST -- failed %s\n", txLockRequest.GetHash().ToString());
                return false;
            }
        } else if (strCommand == NetMsgType::PPTX) {
            uint256 hashTx = tx.GetHash();
            if(CPrivatePay::GetPPTX(hashTx)) {
                LogPrint("privatepay", "DSPP -- Already have %s, skipping...\n", hashTx.ToString());
                return true; // not an error
            }

            CMasternode* pmn = mnodeman.Find(pptx.vin);
            if(pmn == NULL) {
                LogPrint("privatepay", "PPTX -- Can't find masternode %s to verify %s\n", pptx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if(!pmn->fAllowMixingTx) {
                LogPrint("privatepay", "PPTX -- Masternode %s is sending too many transactions %s\n", pptx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
                // TODO: Not an error? Could it be that someone is relaying old PPTXes
                // we have no idea about (e.g we were offline)? How to handle them?
            }

            if(!pptx.CheckSignature(pmn->pubKeyMasternode)) {
                LogPrint("privatepay", "PPTX -- CheckSignature() failed for %s\n", hashTx.ToString());
                return false;
            }

            LogPrintf("PPTX -- Got Masternode transaction %s\n", hashTx.ToString());
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            if (strCommand == NetMsgType::PPTX) {
                LogPrintf("PPTX -- Masternode transaction accepted, txid=%s, peer=%d\n",
                        tx.GetHash().ToString(), pfrom->id);
                CPrivatePay::AddPPTX(pptx);
            } else if (strCommand == NetMsgType::TXLOCKREQUEST) {
                LogPrintf("TXLOCKREQUEST -- Transaction Lock Request accepted, txid=%s, peer=%d\n",
                        tx.GetHash().ToString(), pfrom->id);
                instapay.AcceptLockRequest(txLockRequest);
            }
        ...
        else if (fMissingInputs)
        {
            ...
            if (strCommand == NetMsgType::TXLOCKREQUEST && !AlreadyHave(inv)) {
                ...
                instapay.RejectLockRequest(txLockRequest);
    ...
    else
    {
        ...
        if (found)
        {
            ...
            privatePayClient.ProcessMessage(pfrom, strCommand, vRecv);
            privatePayServer.ProcessMessage(pfrom, strCommand, vRecv);
            mnodeman.ProcessMessage(pfrom, strCommand, vRecv);
            mnpayments.ProcessMessage(pfrom, strCommand, vRecv);
            instapay.ProcessMessage(pfrom, strCommand, vRecv);
