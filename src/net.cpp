...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (usdecoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "usdecoin Core " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. usdecoin Core is probably already running."), addrBind.ToString());
        else
...
void CConnman::RelayTransaction(const CTransaction& tx)
{
    ...
    CPrivatepayBroadcastTx pptx = CPrivatePay::GetPPTX(hash);
    if(pptx) { // MSG_PPTX
        ss << pptx;
    } else if(instapay.GetTxLockRequest(hash, txLockRequest)) { // MSG_TXLOCK_REQUEST
        ss << txLockRequest;
    } else { // MSG_TX
        ss << tx;
    }
...
{
    uint256 hash = tx.GetHash();
    int nInv = static_cast<bool>(CPrivatePay::GetPPTX(hash)) ? MSG_PPTX :
                (instapay.HasTxLockRequest(hash) ? MSG_TXLOCK_REQUEST : MSG_TX);
    CInv inv(nInv, hash);
    {
        LOCK(cs_mapRelay);
