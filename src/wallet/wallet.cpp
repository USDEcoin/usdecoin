...
{
    BOOST_FOREACH(CAmount d, CPrivatePay::GetStandardDenominations())
...
// Recursively determine the rounds of a given input (How deep is the PrivatePay chain for a given input)
int CWallet::GetRealInputPrivatePayRounds(CTxIn txin, int nRounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        if (mdwi == mDenomWtxes.end()) {
            ...
            LogPrint("privatepay", "GetRealInputPrivatePayRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint("privatepay", "GetRealInputPrivatePayRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn txinNext, wtx->vin) {
            if (IsMine(txinNext)) {
                int n = GetRealInputPrivatePayRounds(txinNext, nRounds + 1);
...
int CWallet::GetInputPrivatePayRounds(CTxIn txin) const
{
    LOCK(cs_wallet);
    int realPrivatePayRounds = GetRealInputPrivatePayRounds(txin, 0);
    return realPrivatePayRounds > privatePayClient.nPrivatePayRounds ? privatePayClient.nPrivatePayRounds : realPrivatePayRounds;
}
    ...
    if (!IsCoinBase())
    {
        if (GetDepthInMainChain() == 0 && !isAbandoned() && InMempool()) {
            ...
            if(strCommand == NetMsgType::TXLOCKREQUEST) {
                instapay.ProcessTxLockRequest(((CTxLockRequest)*this));
            }
            if (connman) {
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        const int nRounds = pwallet->GetInputPrivatePayRounds(txin);
        if(nRounds >= privatePayClient.nPrivatePayRounds){
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...
CAmount CWallet::GetAnonymizableBalance(bool fSkipDenominated, bool fSkipUnconfirmed) const
{
    ...
    const CAmount nSmallestDenom = CPrivatePay::GetSmallestDenomination();
    const CAmount nMixingCollateral = CPrivatePay::GetCollateralAmount();
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                nTotal += GetInputPrivatePayRounds(txin);
                nCount++;
            }
        }
    }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int nRounds = GetInputPrivatePayRounds(txin);
                nTotal += pcoin->vout[i].nValue * nRounds / privatePayClient.nPrivatePayRounds;
            }
        }
    }
...
CAmount CWallet::GetNeedsToBeAnonymizedBalance(CAmount nMinBalance) const
{
    ...
    CAmount nNeedsToAnonymizeBalance = privatePayClient.nPrivatePayAmount*COIN - nAnonymizedBalance;
    ...
    if (nNeedsToAnonymizeBalance > CPrivatePay::GetMaxPoolAmount()) nNeedsToAnonymizeBalance = CPrivatePay::GetMaxPoolAmount();
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstaPay) const
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            // do not use IX for inputs that have less then INSTAPAY_CONFIRMATIONS_REQUIRED blockchain confirmations
            if (fUseInstaPay && nDepth < INSTAPAY_CONFIRMATIONS_REQUIRED)
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                } else if(nCoinType == ONLY_PRIVATEPAY_COLLATERAL) {
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(CAmount d, CPrivatePay::GetStandardDenominations()) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseInstaPay) const
{
    // Note: this function should never be used for "always free" tx types like pptx

    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nCoinType, fUseInstaPay);
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(nCoinType == ONLY_DENOMINATED) {
                ...
                if(nRounds < privatePayClient.nPrivatePayRounds) continue;
            }
    ...
    if(nCoinType == ONLY_DENOMINATED) {
        std::vector<CAmount> vecPrivatePayDenominations = CPrivatePay::GetStandardDenominations();
        CAmount nSmallestDenom = vecPrivatePayDenominations.back();
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH(CAmount nDenom, vecPrivatePayDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivatePayRoundsMin, int nPrivatePayRoundsMax)
{
    vecTxInRet.clear();
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int nRounds = GetInputPrivatePayRounds(txin);
            if(nRounds >= nPrivatePayRoundsMax) continue;
            if(nRounds < nPrivatePayRoundsMin) continue;

            BOOST_FOREACH(int nBit, vecBits) {
                if(out.tx->vout[out.i].nValue == vecPrivatePayDenominations[nBit]) {
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    CAmount nSmallestDenom = CPrivatePay::GetSmallestDenomination();
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                // ignore anonymized
                if(GetInputPrivatePayRounds(CTxIn(wtx.GetHash(), i)) >= privatePayClient.nPrivatePayRounds) continue;
            }
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivatePayRoundsMin, int nPrivatePayRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nPrivatePayRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT100000IFMN : ONLY_DENOMINATED);
...
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, fOnlyConfirmed, NULL, false, ONLY_PRIVATEPAY_COLLATERAL);
...
bool CWallet::IsCollateralAmount(CAmount nInputAmount) const
{
    ...
    return  nInputAmount >  CPrivatePay::GetCollateralAmount() &&
            nInputAmount <= CPrivatePay::GetMaxCollateralAmount() &&
            nInputAmount %  CPrivatePay::GetCollateralAmount() == 0;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    if (!GetCollateralTxIn(txinCollateral, nValue)) {
        strReason = "PrivatePay requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    CTxOut txout = CTxOut(nValue - CPrivatePay::GetCollateralAmount(), scriptChange);
...
bool CWallet::GetBudgetSystemCollateralTX(CTransaction& tx, uint256 hash, CAmount amount, bool fUseInstaPay)
{
    CWalletTx wtx;
    if(GetBudgetSystemCollateralTX(wtx, hash, amount, fUseInstaPay)){
        tx = (CTransaction)wtx;
        return true;
    }
    return false;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, CAmount amount, bool fUseInstaPay)
{
    // make our change address
    CReserveKey reservekey(this);
    ...
    CCoinControl *coinControl=NULL;
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, nChangePosRet, strFail, coinControl, true, ALL_COINS, fUseInstaPay);
    if(!success){
...
bool CWallet::CreateTransaction(const vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
                                int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseInstaPay)
{
    CAmount nFeePay = fUseInstaPay ? CTxLockRequest().GetMinFee() : 0;
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstaPay))
                {
                    if (nCoinType == ONLY_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 USDE.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough PrivatePay non-denominated funds for this transaction that are not equal 1000 USDE.");
                    ...
                    if (fUseInstaPay) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 USDE."), sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE));
                        } else {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        privatePayClient.ClearSkippedDenominations();
                    } else {

                        // Fill a vout to ourself
                        // TODO: pass in scriptChange instead of reservekey so
                        // change transaction isn't always pay-to-usdecoin-address
                        CScript scriptChange;
...
int CMerkleTx::GetDepthInMainChain(const CBlockIndex* &pindexRet, bool enableIX) const
{
    ...
    if(enableIX && nResult < 6 && instapay.IsLockedInstaPayTransaction(GetHash()))
        return nInstaPayDepth + nResult;
