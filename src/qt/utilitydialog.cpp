    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("usdecoin Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About usdecoin Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  usdecoin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        setWindowTitle(tr("PrivatePay information"));

        ui->aboutMessage->setTextFormat(Qt::RichText);
        ui->scrollArea->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
        ui->aboutMessage->setText(tr("\
<h3>PrivatePay Basics</h3> \
PrivatePay gives you true financial privacy by obscuring the origins of your funds. \
All the usdecoin in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
PrivatePay uses an innovative process to mix your inputs with the inputs of two other people, without having your coins ever leave your wallet. \
...
These denominations are 0.01 USDE, 0.1 USDE, 1 USDE and 10 USDE -- sort of like the paper money you use every day.</li> \
...
Each time the process is completed, it's called a \"round.\" Each round of PrivatePay makes it exponentially more difficult to determine where your funds originated.</li> \
...
Consequently, users who have backups disabled will also have PrivatePay disabled. <hr>\
For more info see <a href=\"https://usdecoinpay.atlassian.net/wiki/display/DOC/PrivatePay\">https://usdecoinpay.atlassian.net/wiki/display/DOC/PrivatePay</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("usdecoin Core is shutting down...") + "<br /><br />" +
