    ...
    platformStyle(platformStyle)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_PRIVATEPAY_ROUNDS, 88);
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            int nRounds = pwalletMain->GetInputPrivatePayRounds(txin);
            if (coinControl->fUsePrivatePay && nRounds < privatePayClient.nPrivatePayRounds) {
                ...
                coinControl->fUsePrivatePay = false;
            }
        }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    BOOST_FOREACH(const COutput& out, vOutputs) {
        ...
        // Add inputs to calculate InstaPay Fee later
        if(coinControl->fUseInstaPay)
    ...
    if (nQuantity > 0)
    {
        ...
        // InstaPay Fee
        if (coinControl->fUseInstaPay) nPayFee = std::max(nPayFee, CTxLockRequest(txDummy).GetMinFee());
        ...
        if (nPayAmount > 0)
        {
            ...
            // PrivatePay Fee = overpay
            if(coinControl->fUsePrivatePay && nChange > 0)
            {
                nPayFee += nChange;
                nChange = 0;
            }
    ...
    int nDisplayUnit = BitcoinUnits::USDE;
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show usdecoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            if (nRounds >= 0 || fDebug) itemOutput->setText(COLUMN_PRIVATEPAY_ROUNDS, strPad(QString::number(nRounds), 11, " "));
            else itemOutput->setText(COLUMN_PRIVATEPAY_ROUNDS, strPad(QString(tr("n/a")), 11, " "));
