...
void WalletModel::pollBalanceChanged()
{
    ...
    if(fForceCheckBalanceChanged || chainActive.Height() != cachedNumBlocks || privatePayClient.nPrivatePayRounds != cachedPrivatePayRounds || cachedTxLocks != nCompleteTXLocks)
    {
        fForceCheckBalanceChanged = false;

        // Balance and number of transactions might have changed
        cachedNumBlocks = chainActive.Height();
        cachedPrivatePayRounds = privatePayClient.nPrivatePayRounds;
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered usdecoin address / amount:
...

    {
        ...
        if(recipients[0].fUseInstaPay && total > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN){
            Q_EMIT message(tr("Send Coins"), tr("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 USDE.").arg(sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)),
                         CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, nChangePosRet, strFailReason, coinControl, true, recipients[0].inputType, recipients[0].fUseInstaPay);
        transaction.setTransactionFee(nFeeRequired);
        if (fSubtractFeeFromAmount && fCreated)
            transaction.reassignAmounts(nChangePosRet);

        if(recipients[0].fUseInstaPay) {
            if(newTx->GetValueOut() > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
                Q_EMIT message(tr("Send Coins"), tr("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 USDE.").arg(sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)),
                             CClientUIInterface::MSG_ERROR);
                return TransactionCreationFailed;
            }
            if(newTx->vin.size() > CTxLockRequest::WARN_MANY_INPUTS) {
                Q_EMIT message(tr("Send Coins"), tr("Used way too many inputs (>%1) for this InstaPay transaction, fees could be huge.").arg(CTxLockRequest::WARN_MANY_INPUTS),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal usdecoin:URI (usdecoin:XyZ...?message=example)
            {
        ...
        if(!wallet->CommitTransaction(*newTx, *keyChange, g_connman.get(), recipients[0].fUseInstaPay ? NetMsgType::TXLOCKREQUEST : NetMsgType::TX))
